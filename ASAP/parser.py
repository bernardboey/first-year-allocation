...
def validate_student_data(students_df):
    ...
    for column in LIVING_PREFERENCE_COLUMNS:
        try:
...
def add_students(students_df):
    ...
    for i in range(len(students_df)):
        ...
        student_data = {var: students_df.loc[i, col_name] for var, col_name in COLUMN_NAMES}
...
def generate_temp_results(suites, csv_path):
    ...
    suites_data = {
        ...
        "Sleep Prefs": [[student.data.sleep_pref for student in suite.students] for suite in suites],
        "Sleep Prefs Similarity": [scoring.sleep_pref_score(suite.students) for suite in suites],
        "Suite Prefs": [[student.data.suite_pref for student in suite.students] for suite in suites],
        "Suite Prefs Similarity": [scoring.suite_pref_score(suite.students) for suite in suites],
        "Cleanliness Prefs": [[student.data.cleanliness_pref for student in suite.students] for suite in suites],
        "Cleanliness Prefs Similarity": [scoring.cleanliness_pref_score(suite.students) for suite in suites],
        "Alcohol Prefs": [[student.data.alcohol_pref for student in suite.students] for suite in suites],
